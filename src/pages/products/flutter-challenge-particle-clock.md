...
2. Could we spike curiousity enough to make you long for the next design iteration? Or perhaps feel a bit sad when a design you liked is forever gone?
3. Could background shapes, colors and animations be randomized in such a way to make it a) look good, b) random enough to satisfy 1 & 2, and c) not distract too much from you actually telling the time?
...
In addition to the clock hands, a particle could also spawn as noise. It would then be emitted in a random direction from the center. At birth, all particles are also assigned a random velocity, color, size and painting style (fill or stroke). This to make the clock a bit more interesting to look at.

![An early version of the Flutter Particle Clock](/img/flutter-clock/flutter-clock-early-version.jpeg)

An early version of the clock. This one has quarter markers, and some particles have speed marks. The first versions of the clock was just particles, but I didn't bother to take any screenshots of those.

## Adding Layers With Flutter Widgets

Up until this point, everything was drawn using a single `CustomPainter` widget. The clock looked fine, but it was hard to tell the time. In addition, the background was a single color and looked plain boring.

Flutter is great at layout. It's a toolkit built for user interfaces, after all. To stack a bunch of widgets on top of each other, you just wrap them in a `Stack` widget. The particle clock ended up with a scene widget responsible for building 3 main layers:
...
Here's same picture as above, but without the overlays.

## Animating in Sync With Time

I had the idea early on that it would be cool if the animation happened in sync with the clock ticking. The solution that ended up in the final version is very simple. Getting there was not. Initally I made it into a much bigger problem than it was, and tried all sorts of weird hacks to make it work.
...
This code runs on each animation tick. By using `DateTime.now().millisecond` as a fraction of a millisecond in combination with `Curves`, we get a value between 0 and 1. The `max` function makes sure that number stays above 0.2 to always keep the particles moving with each tick.
...
To get something to start with, I used the [ColourLovers API](https://www.colourlovers.com/) to fetch some of the most liked palettes their users had crowd sourced. Off the bat, many of the palettes had poor contrast ratios between the colors. I solved this by creating a script that filtered the array of palettes, based on the [WCAG Contrast](https://www.w3.org/TR/UNDERSTANDING-WCAG20/visual-audio-contrast-contrast.html) guidelines. Once filtered, the list only contained palettes where there were at least one color combination with a contrast ratio greater than or equal to 4.5.
...
Initially I wasn't sure how to avoid to emit noise particles in the directions of the clock hands, but knew it had to be done. I solved it with a bit of brute force code after giving up on searching for a mathematical solution (of course there is one, only I'm not patience enough to find it).
